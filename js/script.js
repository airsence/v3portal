...
const BASE_URL = "http://api.airsence.com:9000/";
...
const UPDATE_URL = BASE_URL + "updateprofile";
const PROFILE_URL = BASE_URL + "getprofile";
...
function onMainLoad(page) {
  ...
  switch (pageType) {
    case "portal":
      GET_LATEST_URL = BASE_URL + "getlatestlogin";
      SENSOR_URL = BASE_URL + "getlocationlogin";
      POLLUTANT_URL = BASE_URL + "getpollutantlogin";
      try {
      ...
      break;
    case "main":
      GET_LATEST_URL = BASE_URL + "getlatest";
      SENSOR_URL = BASE_URL + "getlocation";
      POLLUTANT_URL = BASE_URL + "getpollutant";
      break;
    default:
      GET_LATEST_URL = BASE_URL + "getlatest";
      SENSOR_URL = BASE_URL + "getlocation";
...
function updateChart(pollutant) {
  ...
  try {
    if (pollutantJSON != null && pollutantJSON.length > 0 && enoughPoint) {
      ...
      for (let i = 0; i < numPoint; i++) {
        if (dict[date[i]]) {
          value[i] = Math.max(0, dict[date[i]] * unitConversionMul);
        }
      }
