...
function changeTime(){
    ...
    min = checkTime(min);
    hour = checkTime(hour);
    time.innerHTML = "Last Updated: " + monthName[month] + " " + date + ", "+ hour+ ":" + min + " ";
    ...
    */
    markers[i].marker.addListener('click',function(){
       ...
       for(var i = 0; i < markers.length; i++){
        ...
        document.getElementById("aqhi").className = "parammenu-button " + markers[i].color;
        changeContent("aqhi");
        changeButtonColor();
...
function changeButtonColor(){
    ...
    for(var i = 0; i < buttons.length; i++){
        ...
        eval("checkColor(pollutionJSON[pollutionJSON.length-1]." + buttons[i].id + ",buttons[i].id,threshold)");
    };
}
function checkColor(value,id,threshold){
    var button = document.getElementById(id);
    document.getElementById(id+"-unit").innerHTML = parseFloat(value).toFixed(1);
